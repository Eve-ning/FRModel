...
def settings():
        ...
        if st.sidebar.checkbox("Random Cat Image. Courtesy of The Cat API!"):
            ...
            def read_file_uploader(img):
                if img:
                    ...
                    ar = np.asarray(read)[..., :3]
                    ar = skimage.transform.resize(ar,
                                                  [image_rescale, int(image_rescale * ar.shape[1] / ar.shape[0]), 3])
                    f = Frame2D(ar, Frame2D.CHN.RGB)

                    return f
                return None

            img_gray = st.sidebar.file_uploader("Upload Image here", [".jpg", ".png"])
            f = read_file_uploader(img_gray)
            if f is None:
                st.stop()

    glcm_rad = st.sidebar.slider("Radius of GLCM", 2, 25, 2)
    glcm_bins = st.sidebar.slider("Bins of GLCM", 2, 24, 8)
    step_size = st.sidebar.slider("Step Size of GLCM", 1, 10, 1)
    hist_bins = st.sidebar.slider("Histogram Bins", 10, 250, 50)
    ...
    plm_fp = int(st.sidebar.slider("Footprint %", 0, 25, 3) * img_len / 100)
    plm_dist = int(st.sidebar.slider("Minimum Distance %", 0, 25, 10) * img_len / 100)
    plm_marker_size = st.sidebar.slider("Marker Size", 1, 10, 3)
    plm_thres_rel = st.sidebar.slider("Threshold %", 0, 100, 40) / 100
